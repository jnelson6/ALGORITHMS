...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        else if(key < (y->key())){  //new node has less value
            y->left = z;
        }
        else{
            y->right = z;
            z->left = NULL;    //set the pointers
            z->right = NULL;
            z->color = RED;
            insert_fixup(z); // to balance tree.
        }
}

        //const V& value = key_value.second;
        //Node<K, V> *z ;//= new Node<pair., value>;              // or pair<K,V>.key and .value OR Node<key,value> *z;
  ...
  /*      while(x != NULL){               //case 1
        ...
        z->left = NULL;                 /// SHOULD THESE NULL's BE NULL NODES???? NOT JUST NULL replace with Node()??
        z->right = NULL;
        z->color = RED;
        insert_fixup(z);
...
/*
        ...
        if (x == NULL){
            z->node_ptr = Node(key, key_value.second);
        } 
  */
        /* Otherwise, recur down the tree */
   /*     if (key < x->key) { 
            Node<K,V> *lchild = insert(x->left, key); 
            x->left  = lchild; 
    ...
    { 
        Node<K,V> *rchild = insert(node->right, key); 
        x->right  = rchild; 
  
        // Set parent of root of right subtree 
        rchild->parent = x; 
    } 
    x->left = NULL;                 /// SHOULD THESE NULL's BE NULL NODES???? NOT JUST NULL replace with Node()??
    x->right = NULL;
    x->color = RED;
    insert_fixup(x)
*/
...
private:
    ...
    void left_rotate(Node<K, V> *x) {
        ...
        x->right = y->left;         // turn y's left subtree 
                                    //  intos x's right subtree
        //if(y->left != NULL){
        //    y->left->parent = x;
        //}
    ...
    void right_rotate(Node<K, V> *x) {
        ...
        x->left = y->right;         //turn y's right subtree 
                                    //  intos x's left subtree
        //if(y->right != NULL){
        //    y->right->parent = x;
        //}
        y->parent = x->parent;      // link x's parent to y
    ...
    int height(Node<K, V> *node) const {
        ...
        long lh = 0;
        long rh = 0;
        if (node == NULL && node->parent == NULL){      // node value == null && parent ?
            lh = -1;
            rh = -1;
            ...
            if( node->right != NULL){               // MAYBE do not need this IF statement
                rh +=1;
                height(node->right);
            }
    ...
    int diameter(Node<K, V> *node) const {
        ...
        return 0;
            //  go in left if tree is left skewed or go in right if tree is right skewed 
            //  or go from left to right via the root and finally take out the maximum of three
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        long l = level;
        long w = 0;
        long n = internal_node_count(node) + leaf_count(node);
        long t = std::log2(n);
        if( l == 0){
            w = 1;
        }
